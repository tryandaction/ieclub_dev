...
│
└── testing/                             # 🧪 测试脚本
    ├── Test-Auth-Simple.ps1             # 认证流程测试（登录、获取个人信息等）
    ├── Test-Auth-Full.ps1               # 完整认证流程测试（包含5个测试用例）
...
**简单测试**（4个测试用例）：
测试认证相关功能（登录、获取个人信息、错误密码验证、发送验证码）：

```powershell
cd C:\universe\GitHub_try\IEclub_dev
.\scripts\testing\Test-Auth-Simple.ps1
```
...
```powershell
cd C:\universe\GitHub_try\IEclub_dev
.\scripts\testing\Test-Auth-Full.ps1
```
