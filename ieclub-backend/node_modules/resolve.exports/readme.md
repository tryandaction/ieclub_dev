...
Successful resolutions will always result in a `string` or `string[]` value. This will be the value of the resolved mapping itself â€?which means that the output is a relative file path.
...
When no fields were found, `undefined` is returned. If you wish to mimic Node.js behavior, you can assume this means `'index.js'` â€?but this module does not make that assumption for you.
...
* `false` â€?if the package author decided a file should be ignored; or
* your `options.browser` string value â€?but made relative, if not already
