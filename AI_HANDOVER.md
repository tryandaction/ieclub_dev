...
**修复** (`ieclub-frontend/pages/auth/index.wxss`):
1. **底部显示问题**
   - 增加页面底部padding从120rpx到200rpx
   - 装饰容器从fixed改为absolute定位
   - 设置装饰层z-index为-1确保在内容下方
   - 降低装饰高度和不透明度

2. **布局优化**
   - 调整Logo区域padding，顶部减少到40rpx
   - 优化表单项间距为32rpx
   - 增加按钮间距到40rpx
   - 用户协议字体增大到26rpx，提高可见度
