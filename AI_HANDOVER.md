...
> **交接时间**: 2025-11-27 23:10  
> **当前版本**: V3.0 - 前后端兼容性修复完成 🚀  
> **代码状态**: ✅ 话题发布、评论系统、详情页显示全部正常  
> **Git分支**: main (生产)  
> **部署状态**: ✅ 生产环境稳定运行  
> **最新工作**: 修复发布500错误、评论API、头像显示、时间格式化
...
**AI 开发提示词**：
```
请完善 IEclub 的发布功能。当前发布页面 Publish.jsx 只有基础表单，需要：
1. 根据选择的话题类型（我来讲/想听/项目/分享）动态显示不同的表单字段
2. 每种类型有独立的必填字段验证
3. 后端 topicController.js 需要存储额外字段
4. 同步更新小程序端 publish 页面
```
...
**AI 开发提示词**：
```
请完善 IEclub 的活动管理功能。当前活动模块基础功能已有，需要增加：
1. 活动发布表单增加更多字段（地点、人数限制、报名截止时间）
2. 报名时检查人数限制，满员后加入候补
3. 生成签到二维码，支持扫码签到
4. 活动开始前30分钟发送提醒通知
```
...
```
请完善 IEclub 的管理后台。需要：
1. 话题审核列表（待审核、已通过、已拒绝）
2. 用户管理（禁言、封禁、角色设置）
3. 数据统计面板（用户增长、内容发布、活跃度）
```
